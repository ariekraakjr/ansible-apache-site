...
<?php
	...
	foreach ($track_list as &$track) { $track = preg_replace('/_/',' ',$track); }
	$page_title = join(', ',$track_list);
?>
	...
	<body style="margin:0px;">
		...
		<script type="text/javascript">
			...
			gv_options.center = [<?=($_REQUEST['center']?$_REQUEST['center']:'45.4,-121.7')?>];  // [latitude,longitude] - be sure to keep the square brackets
			gv_options.zoom = <?=($_REQUEST['zoom']?$_REQUEST['zoom']:12)?>;  // higher number means closer view; can also be 'auto' for automatic zoom/center based on map elements
			gv_options.map_type = 'OPENSEAMAP';  // popular map_type choices are 'GV_STREET', 'GV_SATELLITE', 'GV_HYBRID', 'GV_TERRAIN', 'GV_OSM', 'GV_TOPO_US', 'GV_TOPO_WORLD' (https://www.gpsvisualizer.com/misc/leaflet_map_types.html)
			gv_options.map_opacity = 0.70;  // number from 0 to 1
			...
			gv_options.profile_options = { visible:false, icon:true, units:'metric', filled:true, y_min:null, y_max:null, height:120, width:690, gap_between_tracks:true, waypoints:true }; // see https://www.gpsvisualizer.com/tutorials/profiles_in_maps.html
			...
			gv_options.marker_list_options = {};  // options for a dynamically-created list of markers
			  ...
			  gv_options.marker_list_options.include_tickmarks = true;  // true|false: are distance/time tickmarks included in the list?
			  ...
			  gv_options.marker_list_options.url_links = true;  // true|false: do the names in the list become instant links to the markers' URLs?
			...
			gv_options.marker_filter_options = {};  // options for removing waypoints that are out of the current view
			  gv_options.marker_filter_options.enabled = true;  // true|false: should out-of-range markers be removed?
		...
		<script type="text/javascript">
			function GV_Map() {
				...
				<?php
					$path_to_tracks = '/gps/gpx/';
					$colors = array('blue','green','purple','red','orange','yellow','brown'); $color_index = 0;
					$track_list = array();
					...
					if (!count($track_list)) {
//						print "alert('To view a track, add its name to the URL\\'s \\'track\\' parameter.')\n";
					
//					
//					foreach ($track_list as $track) {
//						$track = preg_replace('/\.gpx$/i','',$track);
//						$GPX_PATH = $path_to_tracks.$track.".gpx";
						...
						foreach($fileList as $filename){
							...
							if(is_file($filename)){
								...
								//echo '<a href=\'', $filename, '\'>', $track, '</a><br>'; 
								$GPX_PATH = $path_to_tracks.$track.".gpx";
				?>
								...
								gv_options.dynamic_data.push({
									url:'<?=$GPX_PATH?>',
									autozoom:true,
									track_options:{color:'<?=$colors[$color_index++ % count($colors)]?>',width:'4',opacity:'0.8'}
								});
//<nobr><a href="map_input?cumulative_distance=1&form=svg&tickmark_zero=1&units=metric&form:remo  tickmark_interval=1%20km
				<?php
							}   
						}
					} else {
